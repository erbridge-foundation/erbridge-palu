...
- [ ] 1.4 Make `SystemRef` `Serialize` (echo `to`/`from` as-sent — bare string or
      number, original casing).
- [ ] 1.5 Confirm the `utoipa` schema for the untagged result entry renders as a
      clean `oneOf`/`allOf`; record the outcome in design.md.
- [ ] 1.6 DTO unit tests: fan-out request deserializes (header + `to` list);
...
- [ ] 2.1 Reject an empty `to[]` and a `to[]` over the sanity cap with
      `AppError::InvalidParam` (request-level), before any route is computed.
- [ ] 2.2 Rewrite `route_system`: load graph + EVE-Scout snapshots once; resolve
      ...
      and `to`-bounds gates.
- [ ] 2.3 Update the `#[utoipa::path]` annotation: fan-out request body, `200`
      returning the `{ from, results }` shape, `400` for the header/bounds tier;
      prose explaining per-destination errors.
- [ ] 2.4 Handler unit tests: multi-destination in request order; bad shared `from`
...
- [ ] 5.1 Add the committed fan-out load body (`tests/load/fanout-wh.json`): one
...
- [ ] 5.2 Factor the fixture-boot scaffolding (background server, wait `/health`,
      teardown) shared by the HURL runner and the load-test recipes.
- [ ] 5.3 Add `just load-test` driving `oha` in the **foreground** (live TUI;
      server backgrounded; no stdout pipe) against the fixture server with the
      shared body. Hint to `cargo install oha` if missing.
- [ ] 5.4 Add `just load-test-plain` (same as 5.3 but `--no-tui` for a capturable
      plain-text summary).
- [ ] 5.5 Confirm neither load recipe is wired into CI or the `check` recipe; add a
      recipe comment noting the hot-graph upper-bound caveat from design.md.

## 6. Verification

- [ ] 6.1 Run `just check` (fmt, clippy, test, hurl) and confirm green.
- [ ] 6.2 Manually exercise `just load-test` once to confirm the TUI renders
