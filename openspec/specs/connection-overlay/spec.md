# connection-overlay Specification

## Purpose

Augment the base gate graph per request with typed connections — user-supplied
`bridge` and `wormhole` edges, plus EVE-Scout Thera/Turnur wormhole signatures —
and apply the default Zarzakh transit exclusion, all without mutating the base
graph. Bridge and wormhole edges are gated by independent use-flags.
