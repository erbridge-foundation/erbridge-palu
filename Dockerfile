...
RUN touch src/main.rs src/lib.rs && cargo build --release --bin erbridge-palu
...
RUN apt-get update \
    ...
    && rm -rf /var/lib/apt/lists/*
COPY --from=builder /app/target/release/erbridge-palu /usr/local/bin/erbridge-palu
# Persisted SDE cache lives here; mount a volume to survive restarts.
ENV PALU_CACHE_DIR=/var/cache/erbridge-palu
EXPOSE 5001
ENTRYPOINT ["erbridge-palu"]
